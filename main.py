...
app = Flask(__name__)
app.config["SQLALCHEMY_DATABASE_URI"] = "sqlite:///daily-nutrition.db"
db = SQLAlchemy()
db.init_app(app)
...
api_url = os.environ['NUTRITION_API_URL']
headers = {
    'X-Api-Key': os.environ['NUTRITION_API_KEY']
}
DEF_BUTTON_COLOR = ('white', 'black')


'''
May not be the original purpose, but this code is being used to resize an image 
for use with PySimpleGUI (tkinter) button graphics
...
def GraphicButton(text, key, image_data, color=DEF_BUTTON_COLOR, size=(100, 50)):
    return sg.Button(text, image_data=resize_base64_image(image_data, size), button_color=color, font='Any 15',
                     pad=(0, 0), key=key, border_width=0)


# Call the Nutrition API, total all the nutrition information, add them to the database, and return a string
def get_nutrition(user_input):
    parameter = {
        "query": user_input
    }
    response = requests.get(api_url, params=parameter, headers=headers)

    # If the API call is successful, total the nutrition information and add them to the database
    if response.status_code == requests.codes.ok:
        ...
        for item in items:
            ...
            with app.app_context():
                ...
                db.session.commit()
        return (f"You've consumed a total of {total_calories} calories, {total_fat}g of fat, {total_protein}g of "
                f"protein, {total_carbs}g of carbs, and {total_sugar}g of sugar!")

    # If the API call fails, return the error code and message
    else:
        return "Error: " + str(response.status_code) + response.text


# Get the total nutrition information for all items added today using the datetime module
def get_all_today():
    total_calories = 0
    total_fat = 0
    total_protein = 0
    ...
    total_sugar = 0
    with app.app_context():
        # Query the database for all items added today
        items = Nutrition.query.filter_by(date=dt.datetime.now().strftime("%m-%d-%Y")).all()
        if len(items) == 0:
            return "No items added today!"
        for item in items:
            total_calories += item.calories
            total_fat += item.fat
        ...
        return (f"You've consumed a total of {total_calories} calories, {total_fat}g of fat, {total_protein}g of "
            f"protein, {total_carbs}g of carbs, and {total_sugar}g of sugar today!")


# Return all items added to the database in the console
def get_all_time():
    with app.app_context():
        items = Nutrition.query.all()
        print("ID\tName\t\tDate\tCalories\tFat\tProtein\tCarbs\tSugar")
        for item in items:
            print(f"{item.id}\t\t{item.name}\t{item.date}\t{item.calories}\t{item.fat}\t{item.protein}\t{item.carbs}\t{item.sugar}")


# The main function that creates the GUI and handles all the events
def ShowMeTheButtons():
    # Define the layout
    sg.theme('Black')

    frame_layout = [[sg.Text('What have you eaten today?\nEx: I ate 2 scrambled eggs and a chocolate pancake\n'
                             'Note: The default serving size is 100g.', size=(45, 3))],
                    [sg.Input(key='-IN-')],
                    [sg.Text(size=(45, 3), key='-OUT-')],
                    [GraphicButton('Add', '-ADD-', button64),
                        GraphicButton('Delete All', '-DELETE_ALL-', red_pill64),
                        GraphicButton('All Today', '-ALL_TODAY-', green_pill64),
                        GraphicButton('All Time', '-ALL_TIME-', orange64),
                        GraphicButton('Exit', '-EXIT-', red_pill64)]]

    layout = [[sg.Frame('Nutrition Tracker', frame_layout, font=(
        'any 18'), background_color='black')]]

    window = sg.Window('Nutrition Tracker', layout,
                       grab_anywhere=True,
                       keep_on_top=True,
                       no_titlebar=True,
    ...
    while True:
        ...
        # Add button
            ...
            response = get_nutrition(values['-IN-'])
            window['-OUT-'].update(response)
            window['-IN-'].update("")
        ...
        # Delete All button
            ...
            with app.app_context():
                ...
                db.session.commit()
            window['-OUT-'].update("All items have been deleted!")

        # All Today button
        elif event == '-ALL_TODAY-':
            window['-OUT-'].update(get_all_today())

        # All Time button
        elif event == '-ALL_TIME-':
            get_all_time()
            window['-OUT-'].update("The table is displayed in the console!")

    window.close()

# Define the Base64 images for the buttons and start the program
